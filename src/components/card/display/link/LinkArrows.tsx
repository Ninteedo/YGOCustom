...
export function LinkArrows({arrows, contents}: LinkArrowProps) {
  const arrowSources: string[] = linkArrowOrdered.map((arrow) => arrows.includes(arrow) ? LinkArrowFilled : LinkArrowEmpty);
...
function linkArrowStyle(arrow: LinkArrow): CSSProperties {
  const positions: Record<LinkArrow, CSSProperties> = {
    ...
    [LinkArrow.RIGHT]: { top: "50%", right: "-16px", transform: "translateY(-50%)" },       // Right center
    [LinkArrow.BOTTOM_RIGHT]: { bottom: "0", right: "0", transform: "translate(50%, 50%)" }, // Bottom right
    [LinkArrow.BOTTOM]: { bottom: "-8px", left: "50%", transform: "translateX(-50%)" },   // Bottom center
    [LinkArrow.BOTTOM_LEFT]: { bottom: "0", left: "0", transform: "translate(-50%, 50%)" }, // Bottom left
    [LinkArrow.LEFT]: { top: "50%", left: "0", transform: "translateY(-50%)" },         // Left center
  ...
  return {
    ...positions[arrow],
    position: "absolute",
    transform: `${positions[arrow].transform} rotate(${linkArrowAngle(arrow)}deg) scale(1.3)`,  // Add rotation based on arrow direction
